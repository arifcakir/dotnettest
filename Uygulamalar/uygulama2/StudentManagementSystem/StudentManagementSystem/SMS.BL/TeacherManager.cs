...
namespace SMS.BL
{
    public class TeacherManager : BaseManagement, ITeacherManager
    {
        ...
        public async Task<ResultSpec<int>> Add(TeacherAddUpdateModel teacherAddUpdateModel)
        {
            ...
            var specResult = TeacherAddUpdateSpec.IsSatisfiedBy(teacherAddUpdateModel, TeacherDomainConstants.MinAgeForTeacher, 
                                                                TeacherDomainConstants.MaxAgeForTeacher, today);
            ...
            if (specResult.OverallResult)
            {
                    ...
                    new TransactionOptions { IsolationLevel = IsolationLevel.ReadCommitted }))
                {
                    ...
                    var newTeacher = await _asyncTeacherRepository.Insert(teacher);

                    await _asyncTeacherRepository.Save();

                    scope.Complete();

                    return Results.WithOkSpec<int>(newTeacher.Id).AddMessage(Resource.Successful).AddSpec(new SpecificationResult());
                }
            }
            else
            {
